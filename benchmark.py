...
def run_trial(trial_id, max_duration=12.0):
    ...
    rand_yaw = random.uniform(-math.radians(15), math.radians(15))
    ...
    rand_bx = random.uniform(1.0, 1.25)
    rand_by = random.uniform(-0.12, 0.12)
    ...
    while sim_time < max_duration:
        ...
        # Pure vision FSM with terminal strike advance
        state, cmd_vx, cmd_vy, cmd_vyaw, active_mode, trigger_kick = state_machine.update(
            ball_det, goal_det, sim_time
        )

        if state in [SoccerState.TERMINAL_APPROACH, SoccerState.ALIGN_KICK, SoccerState.KICK]:
        ...
        evaluator.evaluate_step(d, trunk_base_id, ball_body_id, foot_site_id, metrics, sim_time, ball_qvel_adr=ball_qvel_adr)
...
def main():
    ...
    for i in range(1, args.trials + 1):
        ...
        m = run_trial(i, max_duration=args.duration)
        results.append(m)
