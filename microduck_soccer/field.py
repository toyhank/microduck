...
def check_ball_field_status(ball_xy, z=0.035):
    ...
    # 1. Goal mouth check: ball crossed the goal line inside the posts and height
    if GOAL_X <= x <= (GOAL_X + GOAL_NET_DEPTH + 0.05):
        if abs(y - GOAL_Y) < (GOAL_WIDTH / 2.0) and (z <= GOAL_CROSSBAR_HEIGHT + 0.05):
            return BallFieldStatus.GOAL

    # 2. Field bounds check
    if (FIELD_X_MIN <= x <= FIELD_X_MAX) and (FIELD_Y_MIN <= y <= FIELD_Y_MAX):
        return BallFieldStatus.IN_BOUNDS

    # 3. Otherwise out of bounds
